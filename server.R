  ...
  output$tb <- renderUI({
    if (is.null(input$file)) {
      ...
      tabsetPanel(
        ...
        ##  Wavelet and norm. energy sequence
        tabPanel(
        ...
        tabPanel(
          "3. CHMM Training",
          h4("3.1. Training Sequence"),
          plotOutput("Trainingplot"),
         
          h4("3.2. Trained Sequence"),
          plotOutput("Trainingplot1"),
          h4("3.3. Trained Parameters"),
...
"),
  ...
  ## Read selected sample from selectfile with colsel
   samplered <- reactive({
    if (is.null(input$file)) {
      return ()
    }
   ...
   ## Read selected sample from selectfile without colsel
   samplered1 <- reactive({
     if (is.null(input$file)) {
       return ()
     }
   ...
   ## Read selected sample from selectfile1 without colsel
   samplered2 <- reactive({
     if (is.null(input$file)) {
       return ()
     }
   ...
   ## Read selected sample from selectfile1 with colsel
   samplered3 <- reactive({
     if (is.null(input$file)) {
       return ()
     }
   ...
   ## Read merged samples with colsel
   ...
   ## Read merged train samples with colsel
  ...
  ## 2. Wavelet Analysis & Energy Sequence
  ## 2.a Select sample to plot wavelet
  ...
  ## 2.3.a normilized average-energy sequence
  normeneseq <- reactive({
    if (input$Wavesize == "256") {
  ...
  ## 2.3.b Render Plot normilized average-energy sequence
   ...
   ## 3.1.a Normilized average-energy Training Sequence
  ...
  ## 3.1 Plot normilized average-energy Training Sequence
  Trainingplot <- reactive({
  ...
  # 3.1.b Render Plot Normilized average-energy Training Sequence
  output$Trainingplot <- renderPlot({
    Trainingplot()
  })
  
  
  ## 3.2 Trained Sequence plot
  Trainingplot1 <- eventReactive(input$action2,{
    if(is.null(input$file1)) {return()}
    if (input$Wavesize == "256"){
    packets2 <- as.data.frame(loadedtrainvectlist())
    packets3 <- as.data.frame(normenetraseq())
    ...
    packets7 <- t(packets6)
    trainseq <- ts(packets7)
    plot(trainseq, xlab = "Sequence",main = "Training Sequence with states",
  ...
  # train seq
  Trainingdata <- eventReactive(input$action2,{
    if (input$Wavesize == "256"){
      if(is.null(input$file1)) {return()}
  ...
  trainedmod <- eventReactive(input$action2,{
    ...
    isolate( if(input$Trainparameters == "3 State Custom parameters"){
      packets4 <- as.data.frame(Trainingdata())
  ...
  ### 3.2.a Render Plot Trained Sequence
  output$Trainingplot1 <- renderPlot({
    Trainingplot1()
  })
  
  ## 3.3 Trained Parameters
  ...
  # 4.2.a Render Plot rms & kurtosis
